...
file_path = get_exp_file_path(
  github_dir = github_dir,
  samples_ID = "Dolgos_PDOs-Dolgos_Tissue_Matching",
  prev_exp = "7_infer_cnv_visualization", 
  pattern = "sce_comb_infer_cnv_scored.rds" # Sce after batch correction for visualization
)
...
# Loupe will automatically get rid of columns with unique value per cell (such as scores)
# But these values will be exported in the final object
colData_to_keep <- c(
  "UMAP1", "UMAP2",
  ...
  "Hirz_Epithelial_Club_score","Hirz_Epithelial_Hillock_score",
  "Hirz_Epithelial_Basal_score","Hirz_Epithelial_Luminal_score","Hirz_Epithelial_Tumor_score",
...
write.csv(colData_table, 
          file = paste0(out_path, "Dolgos_2024_Matched_Tissue_PDOs_metadata.csv"),
...
saveRDS(
  object = sce_comb,
  file = paste0(out_path,"Dolgos_2024_Matched_Tissue_PDOs_sce_final.rds"))
...
writeMM(counts_matrix, 
        file = paste0(out_path, "Dolgos_2024_Matched_Tissue_PDOs_matrix.mtx")
)

# Export features.tsv (gene names)
write.table(rowData(sce_comb)$SYMBOL, 
            file = paste0(out_path, "Dolgos_2024_Matched_Tissue_PDOs_features.tsv"), 
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE
)

# Export barcodes.tsv (cell barcodes)
write.table(colnames(sce_comb), 
            file = paste0(out_path, "Dolgos_2024_Matched_Tissue_PDOs_barcodes.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE
)
...
saveRDS(
  object = seurat_comb,
  file  = paste0(out_path,"Dolgos_2024_Matched_Tissue_PDOs_seurat_final.rds")
)
...
create_loupe(
  ...
  output_dir = out_path,
  output_name = "Dolgos_2024_Matched_Tissue_PDOs_final",
  force = T)
