---
title: '1.Dolgos early passage PDO :  Normalization and rescaling'
...
file_path = get_exp_file_path(
  ...
  pattern = "list_sce_qc.rds")

list_sce = readRDS(file = file_path) 

# Load metadata files
metadata = read.csv("/scicore/home/wykopa75/GROUP/rparmentier/sc_RNAseq/Projects/data/Dolgos_2024/Dolgos_2024_scRNAseq_metadata.csv")

```

```{r Kepp only early passage PDOs}

# Identify early_passage_PDOs
early_passage_PDOs = metadata$Sample_Description[which(metadata$Organoid_Type == "early_passage")]

# Select them in the list
list_sce = list_sce[which(names(list_sce) %in% early_passage_PDOs)]

```

```{r Remove low quality cells identified in QC controls}
...
for (sce in list_sce) {
  ...
  # If less than 100 cells in a sce object, set the minimum nb of cell in a cluster to 20
  if(ncol(sce) < 100){min.size = 20}else(min.size = 200)
  
  # Pre-clustering step
  print(paste("Pre-clustering for", names(list_sce)[sce_id], "ongoing..."))
  clust.sce = quickCluster(sce, min.size = min.size)
  
  # Replace the sizeFactor col in metadata, sizeFactors are here computed "cluster-wise"
