---
title: "1.Song 2022 (PDOs): Quality quality controls"
...
Warning : some have more than one file (sub-pools), some avec Normal/Tumor derived organoids

```{r Load the files}

# List of count table files
count_files <- list.files(
  path = paste0("/scicore/home/wykopa75/GROUP/rparmentier/sc_RNAseq/Projects/Sequencing_Data/Pre_Processed_Data/Song_2022/Organoids_count_matrix/"),
  pattern = ".txt",
  full.names = T)
...
```{r Make list of clean and ID marked sce}
...
for (file in 1:length(count_files)) {
  ...
  # Load the count table for file
  count_file <- count_files[file]
  
  # Sample_name
  GSM_ID = str_extract(count_file, "GSM.{7}") # GSM ID = seven letter following GSM in file name
  patient_ID <- str_extract(count_file, "PR.{4}") # Patient ID = seven letter following GSM in file name
  if (str_detect(count_file, "_N_")){ # If N is detected = normal-derived organoids,
    org_origin = "normal_derived_org"
  }else(org_origin = "tumor_derived_org") # if not (=T) = tumor derived organoids
  
  # Show progress
  print(paste("Generating sce object for sample:", GSM_ID, "| patient:", patient_ID, "| organoid = ", org_origin))
  
  # Extract counts
  count_data <- readr::read_delim(count_file, show_col_types = FALSE)
  colnames(count_data)[1] = "SYMBOL" # rename first column which contains gene names
  counts <- as.matrix(count_data[, -1])  # Create a count matrix without the first columns
  rownames(counts) = count_data$SYMBOL # Set rownames of the matrix as gene names
  rm(count_data) # remove original count_data table
  
  # Create the SCE object
  sce <- SingleCellExperiment(
    assays = list(counts = counts),
    rowData = data.frame("SYMBOL" = rownames(counts))
  )
  
  # Set rownames as SYMBOL gene names
  rownames(sce) = rownames(counts)
  
  # Rename the first column of rowData
  colnames(rowData(sce))[1] = "SYMBOL"
  
  # Renaming the cells according Loupe file frindly format
  colnames(sce) = paste0(colnames(sce), "-", GSM_ID, "_", patient_ID)
  
  # Adding new metadata columns containing sample, patient, organoid origin
  sce$GSM_ID = GSM_ID
  sce$Patient_ID = patient_ID
  sce$org_origin = org_origin
  sce$Sample_Description = paste0(patient_ID, "_", org_origin)
  
  # Store the SCE object in the list
  list_sce[[file]] <- sce
  
  names(list_sce)[file] = paste0(patient_ID, "_", org_origin)
  
}

```

Harmonized (= same values and same order) rowData (genes metadata) and colData (celles metadata) are needed to further combine objects
=> Strangely, all sce have a different number of genes (some with many isoforms of different genes). 
=> Either we consider that non-uniformly detected genes should be set at 0 for the other sce.
=> Or we consider that non-uniformly genes should be excluded from the analysis.
**Warning** : doesn't work as some dataset lack essentials feature such as luminal features (KLK3, AMACR...etc) 
using the intersection will make thos gene disappear fron the combined object and eventually from the atlas

```{r Harmonize rowData and colData slots between each other}

# Check number of genes per sce object 
#  lapply(list_sce, function(x){length(rownames(x))})

# Adding zeros for genes not homogeneously detected in all sce ==> Gives 24 793 genes per sce 
list_sce = extend_and_reorder_SCE(sce_list = list_sce) 

## Keeping only commonly found genes between sce ==> Gives 10242 genes per sce 
# list_sce = restrict_reorder_features_names(sce_list = list_sce)  

# Check whether it worked
#  lapply(list_sce, function(x){length(rownames(x))})

# Set rowData for all sce in the list as the rownames of the count matrix (= SYMBOL)
list_sce <- lapply(list_sce, function(x) {
  # Reset rowData to an empty DataFrame
  rowData(x) <- DataFrame(SYMBOL = rownames(x))
  # Return the modified SingleCellExperiment object
  return(x)
  
})

# No need to harmonize colData as they were all built the same in the previous chunk

```

```{r gather sub-pools of identical samples}

# Create a list with indices of sce coming from the same patient and same organoid origin in order to combine them

list_ind_to_combine = list()
sce_id = 1

for (names in unique(names(list_sce))) {
  
  # For each name of the list_sce (which are unique and corresponding to the patient + organoid origin)
  matching_indices =  which(names(list_sce) == names)
  list_ind_to_combine[[sce_id]] = matching_indices
  
  # Name each element with the name of the patient
  names(list_ind_to_combine)[sce_id] = names(list_sce)[matching_indices[[1]]]
  sce_id = sce_id + 1
  
}

# Then create a list with combined sce according to the indices of the previous list

sce_id = 1
list_sce_comb = list()

for (to_combine in list_ind_to_combine) {
  
  list_sce_to_combine = list()
  j = 1 
  
  for (indice in 1:length(to_combine)) {
    
    # First create a list with sce to combine extracte from list_sce and the list_ind_to_combine indices
    list_sce_to_combine[[j]] = list_sce[[to_combine[indice]]]
    j = j + 1
    
  }
  
  # Combine all the sce in the created list into a single sce by concatenating counts and colData
  
  ## Get total raw counts, first in a list, then combine them into a matrix
  counts_comb = lapply(list_sce_to_combine, function(x) {counts(x)})
  ...
  colData_comb = lapply(list_sce_to_combine, function(x) {colData(x)})
  ...
  sce_comb <- SingleCellExperiment( 
    assays = list(counts = counts_comb),  
    rowData = rowData(list_sce_to_combine[[1]]), # RowData is similar among all elements 
    colData = colData_comb
  ) 
  
  # Rename cells with unique 10x cell barcodes + suffix with dataset and ID
  colnames(sce_comb) = paste0(cell_barcodes_whitelist[1:ncol(sce_comb),1], "-", unique(sce_comb$Sample_Description,"_Song_2022"))
  
  list_sce_comb[[sce_id]] = sce_comb
  
  # Name the list with the unique conbinatioun of patient Id and organoid origi
  names(list_sce_comb)[sce_id] = paste0(unique(sce_comb$Patient_ID),"_",unique(sce_comb$org_origin))
 
  sce_id = sce_id + 1
  
}

list_sce = list_sce_comb
