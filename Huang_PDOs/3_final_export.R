...
file_path = get_exp_file_path(
  github_dir = github_dir,
  samples_ID = "Huang_PDOs",
  prev_exp = "2_cycle_annotation", 
  pattern = "sce_comb_cycle_annotated.rds" # Sce after batch correction for visualization
)
...
coords <- as_tibble(reducedDim(sce_comb, "UMAP_original"))[, 1:2]
...
colData_to_keep <- c(
  "UMAP1", "UMAP2",
  "Sample_Description", "cell_cycle_phase",
  "Sample_ID","Treatment","AR_status" 
)
...
write.csv(colData_table, 
          file = paste0(out_path, "Huang_2023_metadata.csv"),
...
saveRDS(
  object = sce_comb,
  file = paste0(out_path,"Huang_2023_sce_final.rds"))
...
saveRDS(
  object = seurat_comb,
  file  = paste0(out_path,"Huang_2023_seurat_final.rds")
)
...
projections = projections[which(names(projections) == c("UMAP_original"))]
