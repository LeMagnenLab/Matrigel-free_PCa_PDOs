---
title: '4bis.Chen 2021 (Tumor Tissue, epithelial cells only): dimension reduction 
...
out_path = create_exp_folder(
  ...
  exp = "4bis_dimension_reduction_tumor_cells"
)
...
```{r Add broad cell type to sce in the list_sce}

sce_id = 1

for (sce in list_sce) {
  
  # Get common cells and broad_cell_tyope associatde to each sce
  common_cells = which(colnames(sce_comb) %in% colnames(sce))
  sce$broad_type = sce_comb$broad_type[common_cells]
  
  # Remove cells that are not epith
  sce = sce[,which(sce$broad_type == "Epithelial cells")]
...
```{r Select only epithelial cells}

sce_comb = sce_comb[,which(sce_comb$broad_type == "Epithelial cells")]
...
fmnn.out <- suppressWarnings(batchelor::fastMNN( 
  ...
  k = 5, # Number of nearest neighbors (higher the smoother the correction is)
  d = 50))# Number of dimension to retain
...
type_colors <- setNames(
  scales::hue_pal()(length(unique(md$broad_type))),
  unique(md$broad_type)
)

sample_id = 1
plot_list = list()

for (type in unique(md$broad_type)) {
  
  md_other <- subset(md, broad_type != type)
  md_type <- subset(md, broad_type == type)
  ...
  plot <- ggplot() +
    ...
    ggrastr::rasterise(
      geom_point(
        data = md_type,
        aes(x = UMAP_1, y = UMAP_2, fill = broad_type), 
        alpha = 0.8, size = 2.5, shape = 21, color = "grey10", stroke = 0.2),
      dpi = 100, scale = 0.3
    ) +
...
plot <- ggplot() +
  ...
  ggrastr::rasterise(
    ...
    dpi = 300, scale = 1) +
  scale_fill_manual(values = type_colors) +  # Apply the custom color palette
  theme_classic() +
  guides(
    alpha = "none",
    size = "none"
...
# Feaature plots
plotUMAP(object = sce_comb,colour_by = "ITGA2")
...
plotExpression(
  ...
  x = "broad_type", # Split by SampleDescription
  colour_by = "broad_type",  # Color points by SampleDescription
  show_smooth = TRUE) + 
  theme(
