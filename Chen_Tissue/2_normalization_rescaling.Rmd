---
title: '2.Chen 2021 (Tumor Tissue): normalization and scaling'
...
for (sce in list_sce) {
  ...
  if(ncol(sce) < 100){min.size = ncol(sce)}else(min.size = 20)
...
common_coldata_col = Reduce(intersect, lapply(list_sce_rescaled, function(x) colnames(colData(x))))
list_sce_rescaled = lapply(list_sce_rescaled, function(sce) {
  colData(sce) = colData(sce)[common_coldata_col]
  return(sce)
  })
