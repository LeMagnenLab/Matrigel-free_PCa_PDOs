---
title: '3.Dolgos Early Passage PDOs & Henry_2018 (Normal Tissue) & Tabula_sapiens (Normal Tissue):  dimension reduction'
...
```
```{r Keep only epithalial cells in sce objects}
...
for(sce in list_sce){
  ...
  # Select only eputhelial cells
  epithelial_cells = which(sce$Cell_Type %in% c("Matrigel_PDOs","ECMf_PDOs","Basal","Club", "Luminal", "Hillock", "Other Epithelial"))
  sce = sce[, epithelial_cells]
...
epithelial_cells = which(sce_comb$Cell_Type %in% c("Matrigel_PDOs","ECMf_PDOs","Basal","Club", "Luminal", "Hillock", "Other Epithelial"))
sce_comb = sce_comb[,epithelial_cells]

# Add a column to sce_comb object
#################################

sce_comb$Dataset_Cell_Type = paste0(sce_comb$Dataset,"_",sce_comb$Cell_Type)
...
for (sample in unique(md$Sample_Description)) {
  ...
  sample_dataset = unique(md_sample$Dataset)
  ...
  plot <- ggplot() +
    ...
    ggrastr::rasterise(
      geom_point(
        ...
        fill = "grey", alpha = 0.4, size = 1, shape = 21, color = "grey10", stroke = 0.1),
      dpi = 100, scale = 0.3
    ) +
    ...
    ggrastr::rasterise(
      geom_point(
        ...
        alpha = 0.8, size = 2.5, shape = 21, color = "grey10", stroke = 0.2),
      dpi = 100, scale = 0.3
    ) +
    scale_fill_manual(values = sample_colors) +
    ggtitle(label = paste0(sample_dataset,": ", sample)) +
    theme_classic() +
    guides(
      fill = "none",
      alpha = "none",
...
```{r Facetted UMAP colored per dataset_cell_Type}

sample_id = 1
plot_list = list()

for (dataset_type in unique(md$Dataset_Cell_Type)) {
  
  md_other <- subset(md, Dataset_Cell_Type != dataset_type)
  md_type <- subset(md, Dataset_Cell_Type == dataset_type)
  ...
  plot <- ggplot() +
    ...
    ggrastr::rasterise(
      geom_point(
        data = md_type,
        aes(x = UMAP_1, y = UMAP_2, fill = Dataset_Cell_Type), 
        alpha = 0.8, size = 2.5, shape = 21, color = "grey10", stroke = 0.2),
      dpi = 100, scale = 0.3
    ) +
    scale_fill_manual(values = pal_dataset_cell_type) +
    ggtitle(label = dataset_type) +
    theme_classic() +
    guides(
      fill = "none",
      alpha = "none",
...
ggsave(
  plot = multiple_page_layout,
  filename = paste0(out_path,time_stamp(),"plots_UMAP_Facetted_Dataset_Cell_Type.pdf"),
  device = "pdf",
  width = 210,
  height = 297,
  unit = "mm")
...
```{r Single UMAP colored Cell_Type}
...
plot <- ggplot() +
  ...
  ggrastr::rasterise(
    geom_point(
      ...
      alpha = 0.6, size = 1.3, 
      shape = 21, , color = "gray10", stroke = 0.2
    ),  # Lighter stroke color (adjust as needed)),# Ensure shape supports fill and color
    dpi = 300, scale = 1) +
  scale_fill_manual(values = pal_cell_type) +  # Apply the custom color palette
  theme_classic() +
  guides(
    alpha = "none",
    size = "none"
...
```{r Single UMAP colored Dataset}

md$Dataset <- ifelse(
  test = grepl("Matrigel", md$Cell_Type),
  yes = paste0(md$Dataset, "_", "Matrigel"),
  no = ifelse(
    test = grepl("ECMf", md$Cell_Type),
    yes = paste0(md$Dataset, "_", "ECMf"),
    no = md$Dataset
  )
)
...
plot <- ggplot() +
  ...
  ggrastr::rasterise(
    geom_point(
      data = md,
      aes(x = UMAP_1, y = UMAP_2, fill = Dataset), 
      alpha = 0.6, size = 1.3, 
      shape = 21, , color = "gray10", stroke = 0.1
    ),  # Lighter stroke color (adjust as needed)),# Ensure shape supports fill and color
    dpi = 300, scale = 1) +
  scale_fill_manual(values = pal_dataset) +  # Apply the custom color palette
  theme_classic() +
  guides(
    alpha = "none",
    size = "none"
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_Single_Dataset.pdf"),
  device = "pdf",
  width = 12,
  height = 8,
)
...
plotUMAP(object = sce_comb,colour_by = "LTF")
plotUMAP(object = sce_comb,colour_by = "PIGR")
plotUMAP(object = sce_comb,colour_by = "OLFM4")
plotUMAP(object = sce_comb,colour_by = "VIM")
plotUMAP(object = sce_comb,colour_by = "AR")
plotUMAP(object = sce_comb,colour_by = "VIM")
...
plotExpression(
  sce_comb, 
  features = c("AR","KLK3","AMACR","VIM","PIGR","LTF","KRT5","KRT19"),
  x = "Dataset_Cell_Type", # Split by SampleDescription
  colour_by = "Dataset_Cell_Type",  # Color points by SampleDescription
  show_smooth = TRUE) + 
  theme(
