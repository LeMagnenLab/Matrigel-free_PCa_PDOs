---
title: '3.Dolgos Early Passage PDOS & Chen 2021 (Epithelial cells only): dimension reduction'
...
```{r Facetted UMAP colored per Malignancy}

sample_id = 1
plot_list = list()

for (malignancy_type in unique(md$Malignancy)) {
  
  md_other <- subset(md, Malignancy != malignancy_type)
  md_type <- subset(md, Malignancy == malignancy_type)
...
md$Cell_Type = ifelse(
  test = md$Cell_Type == "Epithelial cells",
  yes = "Parental_Tumor_cells",
  no = md$Cell_Type )

md_PDOs <- subset(md, Cell_Type %in% c("ECMf_PDOs","Matrigel_PDOs"))
md_parental <- subset(md, Cell_Type == "Parental_Tumor_cells")
...
plot <- ggplot() +
  ...
  ggrastr::rasterise(
    geom_point(
      ...
      alpha = 0.6, size = 1.3, shape = 21, color = "grey10", stroke = 0.2),
    dpi = 300, scale = 1
  ) +  
ggrastr::rasterise(
  geom_point(
...
plotExpression(
  ...
  x = "Cell_Type", # Split by SampleDescription
  colour_by = "Cell_Type",  # Color points by SampleDescription
  show_smooth = TRUE) + 
  theme(
