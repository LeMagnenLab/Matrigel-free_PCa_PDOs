...
write.csv(colData_table, 
          file = paste0(out_path, "Dolgos_2024_Early_Passage_Chen_2021_Epithelial_metadata.csv"),
...
saveRDS(
  object = sce_comb,
  file = paste0(out_path,"Dolgos_2024_Early_Passage_Chen_2021_Epithelial_sce_final.rds"))
...
writeMM(counts_matrix, 
        file = paste0(out_path, "Dolgos_2024_Early_Passage_Chen_2021_Epithelial_matrix.mtx")
)

# Export features.tsv (gene names)
write.table(rowData(sce_comb)$SYMBOL, 
            file = paste0(out_path, "Dolgos_2024_Early_Passage_Chen_2021_Epithelial_features.tsv"), 
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE
)

# Export barcodes.tsv (cell barcodes)
write.table(colnames(sce_comb), 
            file = paste0(out_path, "Dolgos_2024_Early_Passage_Chen_2021_Epithelial_barcodes.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE
)
...
saveRDS(
  object = seurat_comb,
  file  = paste0(out_path,"Dolgos_2024_Early_Passage_Chen_2021_Epithelial_seurat_final.rds")
)
...
create_loupe(
  ...
  output_dir = out_path,
  output_name = "Dolgos_2024_Early_Passage_Chen_2021_Epithelial_loupe",
  force = T)
