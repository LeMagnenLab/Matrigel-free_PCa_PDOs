...
file_path = get_exp_file_path(
  github_dir = github_dir,
  samples_ID = "Atlas_PCa_PDOs",
  prev_exp = "6_infer_cnv_visualization", 
...
write.csv(colData_table, 
          file = paste0(out_path,time_stamp(), "Atlas_PCa_PDOs_metadata.csv"),
...
saveRDS(
  object = sce_comb,
  file = paste0(out_path,time_stamp(),"Atlas_PCa_PDOs_sce_final.rds"))
...
writeMM(counts_matrix, 
        file = paste0(out_path,time_stamp(), "Atlas_PCa_PDOs_matrix.mtx")
)

# Export features.tsv (gene names)
write.table(rowData(sce_comb)$SYMBOL, 
            file = paste0(out_path,time_stamp(), "Atlas_PCa_PDOs_features.tsv"), 
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE
)

# Export barcodes.tsv (cell barcodes)
write.table(colnames(sce_comb), 
            file = paste0(out_path,time_stamp(), "Atlas_PCa_PDOs_barcodes.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE
)
...
saveRDS(
  object = seurat_comb,
  file  = paste0(out_path,time_stamp(),"Atlas_PCa_PDOs_seurat_final.rds")
)
...
create_loupe(
  ...
  output_dir = out_path,
  output_name = "Atlas_PCa_PDOs_loupe",
  force = T)
