---
title: '2.Henry_2018 (Healthy Tissue): dimension reduction'
...
```{r Facetted UMAP colored per CellType}

CellType_colors <- setNames(
  paletteer::paletteer_d("colorBlindness::paletteMartin")[2:9],
  unique(md$CellType)
)
...
for (cell_type in unique(md$CellType)) {
  
  md_other <- subset(md, CellType != cell_type)
  md_cell_type <- subset(md, CellType == cell_type)
...
```{r Single UMAP colored CellType}
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_Single_CellType.pdf"),
  device = "pdf",
  width = 12,
  height = 8,
)
