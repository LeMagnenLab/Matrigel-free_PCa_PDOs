---
title: "2bis. Song Tissue (Published object): Dimension reduction"
...
file_path = get_exp_file_path(
  github_dir = github_dir,
  samples_ID = "Song_2022_Tum_Tissue",
  prev_exp = "1bis_normalization_rescaling",
...
for (sample_name in unique(md$Sample_Description)) {
  
  md_other <- subset(md, Sample_Description != sample_name)
  md_sample <- subset(md, Sample_Description == sample_name)
...
# Define a color palette for ID
celltype_colors <- setNames(
  scales::hue_pal()(length(unique(md$ID))),
  unique(md$ID)
)
...
for (celltype_name in unique(md$ID)) {
  
  md_other <- subset(md, ID != celltype_name)
  md_sample <- subset(md, ID == celltype_name)
  ...
  plot <- ggplot() +
    ...
    ggrastr::rasterise(
      geom_point(
        data = md_sample,
        aes(x = UMAP_1, y = UMAP_2, fill = ID), 
        alpha = 0.8, size = 2, shape = 21, color = "gray10", stroke = 0.2 
      ), dpi = 200, scale = 0.6
    ) +
    scale_fill_manual(values = celltype_colors) +  # Apply the custom color palette
    ggtitle(label = celltype_name,
            subtitle = "Batch Correction (FastMNN applied)" ) +
    theme_classic() +
    guides(
      fill = "none",
  ...
  plot_list[[celltype_id]] = plot
  
  celltype_id = celltype_id + 1
